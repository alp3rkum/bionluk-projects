...
def main():
    ...
    while True:
        ...
        if choice == "1":
            ...
            data[identifier] = {"password": encrypt_data(key, password), "description": description}
            save_data(key, data)
            print("Password added.")
        elif choice == "2":
            for identifier, info in data.items():
                if identifier != "_master_password":
                    print(f"Identifier: {identifier}, Password: {decrypt_data(key, info['password'])}, Description: {info['description']}")
        elif choice == "3":
